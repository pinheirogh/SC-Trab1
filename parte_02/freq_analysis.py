def contagem_letras(msg_decifrada):
    ...
    for letra in msg_decifrada.upper():
...
def get_freq_ordenada(msg_decifrada, freq_por_lingua):
    alfabeto = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
...
def pontuacao_por_freq(msg_decifrada, idioma_provavel="en"):

    freq_por_lingua = 'AEOSRINDMUTCLPVGHQBFZJXKWY' if idioma_provavel == "pt" else 'ETAOINSHRDLCUMWFGYPBVKJXQZ'
    freq_ordenada = get_freq_ordenada(msg_decifrada, freq_por_lingua)

    pontuacao = 0
    # Find how many matches for the six most common ALFABETO there are:
    for letra_mais_freq in freq_por_lingua[:6]:
        if letra_mais_freq in freq_ordenada[:6]:
            pontuacao += 1
    # Find how many matches for the six least common ALFABETO there are:
    for letra_menos_freq in freq_por_lingua[-6:]:
        if letra_menos_freq in freq_ordenada[-6:]:
            pontuacao += 1
